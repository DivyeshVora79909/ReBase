...
function readContracts(projectDir) {
  ...
  if (!fs.existsSync(contractPath)) throw new Error(`Missing compiled runtime contract: ${contractPath}`);
  const parsed = JSON.parse(fs.readFileSync(contractPath, "utf8"));
  return { contractPath, contracts: new Map(Object.entries(parsed.tables || {})) };
}

function contextsFromEnvironment(value = process.env.REBASE_CONTEXTS) {
  if (!value) return [];
  let parsed;
  try { parsed = JSON.parse(value); } catch { throw new Error("REBASE_CONTEXTS must be valid JSON"); }
  if (!Array.isArray(parsed)) throw new Error("REBASE_CONTEXTS must be a JSON array");
  return parsed.map((context) => {
    if (!context?.namespace || !context?.database) throw new Error("Every REBASE_CONTEXTS entry requires namespace and database");
    return { namespace: String(context.namespace), database: String(context.database) };
  });
}

async function startServer(options = {}) {
  const environment = options.environment || process.env.NODE_ENV || "development";
  const allowBearer = environment === "production"
    ? false
  ...
  if (!Number.isInteger(port) || port < 0 || port > 65535) throw new Error("REBASE_PORT must be a valid TCP port");
  ...
  const handlers = options.handlers || loadTableHandlers(path.join(projectDir, "table-handlers"), loaded);
  ...
  const configuredContexts = options.contexts
    || contextsFromEnvironment()
    || (defaultContext.namespace && defaultContext.database ? [defaultContext] : []);
  if (!configuredContexts.length && defaultContext.namespace && defaultContext.database) configuredContexts.push(defaultContext);
  if ((!defaultContext.namespace || !defaultContext.database) && configuredContexts.length === 1) {
  ...
  if (environment === "production" && !configuredContexts.length) {
    ...
    throw new Error("At least one configured namespace/database context is required in production");
  }
  runtimeOptions.allowedContexts ||= configuredContexts;
  const runtime = createRuntime({ handlers, providers, queue, stores, contracts: loaded.contracts, options: runtimeOptions });
  const workerStops = [];
  let reconciler;
  let stopReconciler;
  let server;
  ...
  try {
    for (const lane of ["task", "schedule", "webhook"]) {
      workerStops.push(await queue.start(lane, (delivery) => runtime.consume(lane, delivery)));
    }
    reconciler = createReconciler({
      runtime,
      contexts: configuredContexts,
      intervalMs: options.reconcileIntervalMs || Number(process.env.REBASE_RECONCILE_INTERVAL_MS) || 30 * 60 * 1000,
      onError: options.onReconcileError,
    });
    stopReconciler = reconciler.start({ immediate: options.reconcileOnStartup !== false });
